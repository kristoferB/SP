...
object APISP {
  ...
  case class StatusResponse(service: String, instanceID: ID = ID.newID, instanceName: String = "", tags: List[String] = List(), api: SPAttributes = SPAttributes(), version: Int = 1, attributes: SPAttributes = SPAttributes()) extends APISP
  ...
  object StatusResponse {
    ...
    def apply(attr: SPAttributes): StatusResponse = {
      ...
      val id = attr.getAs[UUID]("instanceID").getOrElse(ID.newID)
