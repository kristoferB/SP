...
object BSrunner extends SPService {
  ...
  def props(eventHandler: ActorRef, serviceHandler: ActorRef, operationController: String, BSservice: String) =
    ServiceLauncher.props(Props(classOf[BSrunner], eventHandler, serviceHandler, operationController, BSservice))
}

// Inkluderar eventHandler och namnet på servicen operationController. Skickas med i SP.scala
class BSrunner(eventHandler: ActorRef, serviceHandler: ActorRef, operationController: String, BSservice: String) extends Actor with ServiceSupport with SOPPrinterLogic{
  ...
  def receive = {
    case r @ Request(service, attr, ids, reqID) => {
      ...
      //} else {
        ...
        val sopID = transform(BSrunner.transformTuple._1)
        soppen = sopID
        station = transform(BSrunner.transformTuple._2)
