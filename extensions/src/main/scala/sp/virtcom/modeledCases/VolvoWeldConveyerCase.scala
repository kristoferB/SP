...
case class VolvoWeldConveyerCase() extends CollectorModel {
  ...
  v(name = "vRobot_car", domain = Seq("empty", "productA", "productB"), init = "empty", marked = "empty")
...
//  productX("B")
  ...
  //Robot movements (minus "at")
  val staticRobotPos = Map("atHome" -> Set("atIn"),
    "atIn" -> Set("atWeld"),
    "atWeld" -> Set("atConveyer"),
    "atConveyer" -> Set("atHome"))
  ...
  def productX(X: String) = {
    ...
    op(s"releaseProduct$X", c("vRobot_car", s"product$X", "empty"))
    ...
    op(s"removeProduct$X", conditions = SPAttributes("preGuard" -> s"vOutConveyer$X == running", "postGuard" -> s"vOutConveyer$X == running"))
  }
}
