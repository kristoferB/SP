...
case class RobotScheduleSetupOld(selectedSchedules: List[ID])

import oscar.cp._

class RobotOptimizationOld(ops: List[Operation], precedences: List[(ID,ID)],
...
object VolvoRobotScheduleOld extends SPService {
  ...
  val transformTuple = (
    TransformValue("setup", _.getAs[RobotScheduleSetupOld]("setup"))
  )
  val transformation = transformToList(transformTuple.productIterator.toList)

  def props(sh: ActorRef) = ServiceLauncher.props(Props(classOf[VolvoRobotScheduleOld], sh))
}

class VolvoRobotScheduleOld(sh: ActorRef) extends Actor with ServiceSupport with AddHierarchies {
