...
object StateVariableLogic {
  ...
  case class StateVarInfo(domain: SVDomain,
                          init: Option[SPAttributeValue] = None,
                          goal: Option[SPAttributeValue] = None)

  sealed trait SVDomain
  case class DomainList(domain: List[SPAttributeValue]) extends SVDomain
  ...
  implicit class svLogic(th: Thing) {
    ...
    private def kevVal(domain: SVDomain): (String, SPAttributeValue) = {
      domain match {
