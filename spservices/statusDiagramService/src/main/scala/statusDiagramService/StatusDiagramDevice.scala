...
import sp.statusdiagramservice.{API_Patient => apiPatient}
...
class StatusDiagramDevice extends Actor with ActorLogging {
  ...
  mediator ! Subscribe("status-diagram-service-topic", self)

  var state: Map[String, apiPatient.Patient] = Map()
  var widgetStarted: Boolean = false

  println("STAUTS STARTED")
  ...
  */
  def matchRequests(mess: Try[SPMessage]) = {
    ...
    StatusDiagramComm.extractEvent(mess) map { case (h, b) =>
      b match {
  ...
  */
  def publishOnAkka(header: SPHeader, body: api.StateEvent) {
    ...
    toSend match {
      case Success(v) =>
        println("Published current state to status-diagram-widget-topic")
