...
import sp.roomoverviewservice.{API_Patient => apiPatient}
...
class RoomOverviewDevice extends Actor with ActorLogging {
  ...
  mediator ! Subscribe("room-overview-service-topic", self)
  ...
  */
  def matchRequests(mess: Try[SPMessage]) = {
    ...
    RoomOverviewComm.extractEvent(mess) map { case (h, b) =>
      b match {
  ...
  */
  def updateState(careContactId: String, prop: apiPatient.PatientProperty) {
    ...
    if (widgetStarted) {
      publishOnAkka(SPHeader(from = "roomOverviewService"), api.State(state))
    }
  }
