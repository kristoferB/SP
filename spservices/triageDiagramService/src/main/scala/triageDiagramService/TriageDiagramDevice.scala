...
import sp.triagediagramservice.{API_Patient => apiPatient}
...
class TriageDiagramDevice extends Actor with ActorLogging {
  ...
  mediator ! Subscribe("triage-diagram-service-topic", self)
  ...
  */
  def matchRequests(mess: Try[SPMessage]) = {
    ...
    TriageDiagramComm.extractEvent(mess) map { case (h, b) =>
      b match {
        case api.NewPatient(careContactId, patientData, events) => {
          ...
          if (!patientProperties.isEmpty) {
            ...
            if (widgetStarted) {
              publishOnAkka(SPHeader(from = "triageDiagramService"), api.State(state))
            }
          }
        }
  ...
  */
  def publishOnAkka(header: SPHeader, body: api.StateEvent) {
    ...
    toSend match {
      case Success(v) =>
        println("Published current state to triage-diagram-widget-topic")
