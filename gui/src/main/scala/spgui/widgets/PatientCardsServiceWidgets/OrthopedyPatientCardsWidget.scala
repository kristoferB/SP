...
object OrthopedyPatientCardsWidget {
    ...
    val messObs = BackendCommunication.getMessageObserver(
      mess => {
        mess.getBodyAs[api.PatientProperty].map {
          ...
          case _ => println("THIS WAS NOT EXPECTED IN OrthopedyPatientCardsWidget.")
      }
  ...
  */
  def decodeTriageColor(p: Priority): String = {
    p.color match {
      ...
      case _ =>  {
        println("TriageColor: "+ p.color +" not expected in OrthopedyPatientCardsWidget")
