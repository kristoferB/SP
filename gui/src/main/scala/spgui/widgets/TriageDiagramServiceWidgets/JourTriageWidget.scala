...
object JourTriageWidget {

  send(api.GetState())

  def send(mess: api.StateEvent) {
    val h = SPHeader(from = "JourTriageWidget", to = "TriageDiagramService")
    ...
    val messObs = BackendCommunication.getMessageObserver(
      mess => {
        mess.getBodyAs[api.Event] map {
          ...
          case x => println(s"THIS WAS NOT EXPECTED IN JourTriageWidget: $x")
        }
      }, "triage-diagram-jour-widget-topic"
    )
