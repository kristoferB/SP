...
object SurgeryTriageWidget {

  send(api.GetState())

  def send(mess: api.StateEvent) {
    val h = SPHeader(from = "SurgeryTriageWidget", to = "TriageDiagramService")
    ...
    val messObs = BackendCommunication.getMessageObserver(
      mess => {
        mess.getBodyAs[api.Event] map {
          ...
          case x => println(s"THIS WAS NOT EXPECTED IN SurgeryTriageWidget: $x")
        }
      }, "triage-diagram-surgery-widget-topic"
    )
