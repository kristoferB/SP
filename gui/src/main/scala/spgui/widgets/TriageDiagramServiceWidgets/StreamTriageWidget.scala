...
object StreamTriageWidget {

  send(api.GetState())

  def send(mess: api.StateEvent) {
    val h = SPHeader(from = "StreamTriageWidget", to = "TriageDiagramService")
    ...
    val messObs = BackendCommunication.getMessageObserver(
      mess => {
        mess.getBodyAs[api.Event] map {
          ...
          case x => println(s"THIS WAS NOT EXPECTED IN StreamTriageWidget: $x")
        }
      }, "triage-diagram-stream-widget-topic"
    )
