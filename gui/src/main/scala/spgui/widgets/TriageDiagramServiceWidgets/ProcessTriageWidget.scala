...
object ProcessTriageWidget {

  send(api.GetState())

  def send(mess: api.StateEvent) {
    val h = SPHeader(from = "ProcessTriageWidget", to = "TriageDiagramService")
    ...
    val messObs = BackendCommunication.getMessageObserver(
      mess => {
        mess.getBodyAs[api.Event] map {
          ...
          case x => println(s"THIS WAS NOT EXPECTED IN ProcessTriageWidget: $x")
        }
      }, "triage-diagram-process-widget-topic"
    )
